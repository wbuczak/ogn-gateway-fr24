...
public class FR24Forwarder implements OgnAircraftBeaconForwarder {
    ...
    public void sendToFr24(AircraftBeacon beacon) {
        ...
        try {
            ...
            dos.writeInt(swap((int) (System.currentTimeMillis() / 1000L))); // Unix current timestamp
            dos.writeInt(swap((int) (beacon.getLat() * 10000000))); // 1e-7 deg (south?)
            dos.writeInt(swap((int) (beacon.getLon() * 10000000))); // 1e-7 deg (west?)
            ...
            dos.writeShort(swap((short) ((beacon.getGroundSpeed() * 10) / 3.6))); // Speed from km/h to m/s * 10
            dos.writeShort(swap((short) ((beacon.getTrack() * 360) / 65536))); // Heading: 65536/360deg
            dos.writeShort(swap((short) ((beacon.getTurnRate() * 360) / 65536))); // TurnRate: 65536/360deg - not really
                                                                                  // sure. Seems to depend on flarm???
